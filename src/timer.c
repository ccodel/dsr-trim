...
void timer_init(timer_t *t) {
  FATAL_ERR_IF(t == NULL, "timer_init: timer_t pointer is NULL");
  memset(t, 0, sizeof(timer_t));
}

void timer_record(timer_t *t, timer_type_t ty) {
  FATAL_ERR_IF(t == NULL, "record_time: timer_t pointer is NULL");
  struct timeval *timer = &t->timers[ty];
  gettimeofday(timer, NULL);
}

static unsigned long long timer_get_elapsed_millisecs(timer_t *t, timer_type_t ty) {
  FATAL_ERR_IF(t == NULL, "timer_get_millisecs: timer_t pointer is NULL");
